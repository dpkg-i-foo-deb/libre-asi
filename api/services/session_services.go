...
func LoginInterviewerService(c *fiber.Ctx) error {
	...
	err = database.LoginInterviewerStatement.QueryRow(user.Email).Scan(&queriedUser.Email, &queriedUser.Password)
	...
	pair, err = auth.GenerateJWTPair(user.Email, string(models.INTERVIEWER))
